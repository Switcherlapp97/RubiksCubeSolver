...
namespace RubiksCubeLib.Solver
{
  ...
  public abstract class PatternTable
  {
    ...
    {
      Dictionary<Pattern, Algorithm> patterns = Patterns.ToDictionary(pa => pa.Key.DeepClone(), a => a.Value); // clone
      patterns = patterns.Where(kvp => filter.Filter(p, kvp.Key)).ToDictionary(pa => pa.Key, a => a.Value); // filter
      patterns = patterns.OrderByDescending(k => k.Key.Probability).ToDictionary(pa => pa.Key.DeepClone(), a => a.Value); // order by probability
